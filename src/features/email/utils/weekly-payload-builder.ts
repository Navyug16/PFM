 ...
 * Example: "Sep 07, 2026 – Sep 13, 2026"
 */
const formatWeeklyRangeString = (startDateStr: string, endDateStr: string, locale: string): string => {
  try {
    ...
    console.error('Error formatting weekly date range string:', err)
    return `${startDateStr} – ${endDateStr}`
  }
}

/**
 * PURE WEEKLY EMAIL PAYLOAD BUILDER
 * --------------------------------
 * Assembles a read-only EmailReportPayload for weekly financial dispatches.
 * 
...
export const buildWeeklyEmailPayload = (
  ...
  // 2. Delegate week-over-week comparison directly to comparePeriodSpending
  const comparison = comparePeriodSpending(
    allTxs,
    startDate,
    endDate,
    prevStartDate,
  ...
  // Map category breakdown items directly to EmailCategorySpendingItem (percentage mapped directly)
  ...
  const prevSavings = calculatePeriodSavings(prevTransactions, prevStartDate, prevEndDate)
  ...
  const rawInsights = generateReportInsights({
    ...
    prevSavings,
    dailyAverage: Math.round((totalExpenses / 7) * 100) / 100,
  ...
  // 5. Assemble final trusted payload
  return {
    reportType: 'weekly',
    ...
    period: {
      startDate,
      endDate,
      formattedRange: formatWeeklyRangeString(startDate, endDate, user.locale),
    },
    summary: {
      totalIncome,
      totalExpenses,
